"""Windowed 1-D Kuramoto-Sivashinsky problem with hard initial-condition ansatz."""
...
class KS1DWindowed:
    """KS equation on x in [0, L] with periodic boundary and time-windowed hard IC."""
    ...
    def initial_condition(self, x):
        return (1.0 + jnp.sin(x)) * jnp.cos(x)
    ...
    def derivs_jet(self, params, p_past, w_idx, t, x):
        ...
        u_val, (u_x, u_xx, _u_xxx, u_xxxx) = jet(g_x, (x,), ((1.0, 0.0, 0.0, 0.0),))
        g_t = lambda tt: self.hard_ansatz(params, p_past, w_idx, tt, x)
        _, (u_t,) = jet(g_t, (t,), ((1.0,),))
        return u_val, u_t, u_x, u_xx, u_xxxx

    def pde_residual(self, params, p_past, w_idx, t, x):
        u, u_t, u_x, u_xx, u_xxxx = self.derivs_jet(params, p_past, w_idx, t, x)
        return u_t + self.nonlin_c * u * u_x + self.nu * u_xx + self.mu * u_xxxx
